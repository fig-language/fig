...
This document outlines the syntax, generic capabilities, and principles behind functions in the Nyx programming language, reflecting its core design philosophies of explicitness, readability, and modern style.

---

## 1. Function Syntax

Nyx functions are straightforward to define, emphasizing clarity and type safety.

*   All functions are introduced by the `fn` keyword.
*   Nyx requires explicit declaration of **generic type parameters** using the `[T, U, ...]` syntax; generic inference is not supported for function signatures.
...
Nyx's generic functions promote code reusability without sacrificing type safety or runtime performance.
...
*   Constraints on generic parameters are exclusively expressed using **interfaces**, offering a clear and composable mechanism for defining required capabilities. Nyx does not use `type` or `where` clauses for constraints.
...
Nyx supports const generics in functions, enabling parameterization by compile-time constant values.
...
Adhering to these best practices will help you write clear, maintainable, and idiomatic Nyx functions:
...
*   **Multiple Return Values**: Utilize Nyx's support for multiple return values for functions that logically produce more than one result.

---

## 4. Summary

Nyx functions are meticulously designed to be:
...
This design philosophy ensures that whether you are writing a small utility function or a complex library component, Nyx functions remain readable, maintainable, and easy to document.
